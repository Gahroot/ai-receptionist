...
/**
 * Audio service for real-time PCM16 capture using @siteed/expo-audio-studio
 * and audio mode configuration via expo-audio.
 *
 * Note: The actual recording uses the useAudioRecorder hook from
 * @siteed/expo-audio-studio (must be called from a React component).
...
const audioService = {
   ...
   */
  setupAudioMode: async (_speakerMode = false) => {
    await setAudioModeAsync({
      shouldPlayInBackground: true,
      playsInSilentMode: true,
      // iOS: allowsRecording=true enables PlayAndRecord category
   ...
   */
  setSpeakerMode: async (_speaker: boolean) => {
    await setAudioModeAsync({
      shouldPlayInBackground: true,
      playsInSilentMode: true,
      allowsRecording: true,
